...
export const HomeView = () => {
  ...
  return <div>
    ...
    <div className="grid-2" style={{ marginTop: 12 }}>
      <Card><h3>Quick actions</h3><Button onClick={async () => { const r = await detectExceptions(); setMsg(r.message ?? 'done'); }}>Detect exceptions</Button> <Button onClick={async () => { const r = await resetDemoData(); setMsg(r.message); }}>Reset demo data</Button><Button onClick={async () => { const h = await fetchHealth(); setMsg(`Ping: ${h.status}`); }}>Ping API</Button>{msg ? <p>{msg}</p> : null}</Card>
      <Card><h3>Demo instructions</h3><p>Go to <Link to="/inventory">Inventory</Link>, <Link to="/purchase">Purchase</Link>, <Link to="/logistics">Logistics</Link>, <Link to="/exceptions">Exceptions</Link>.</p></Card>
    </div>
  </div>;
};
...
export const IntegrationsView = () => {
  ...
  return <Card><h3>Connector Runs</h3><Button variant="primary" onClick={async () => { await runConnector('csv'); setLoading(true); await load(); }}>Run Demo Connector</Button>
    ...
    {!loading && !error && runs.length > 0 ? <Table><thead><tr><th>ID</th><th>Connector</th><th>Status</th><th>Started</th><th>Finished</th></tr></thead><tbody>{runs.map((r, i) => <tr key={i}><td>{r.id ?? '-'}</td><td>{r.connector_name}</td><td>{r.status}</td><td>{r.started_at ?? '-'}</td><td>{r.ended_at ?? '-'}</td></tr>)}</tbody></Table> : null}
  </Card>;
};
...
export const ExceptionsView = () => {
  ...
  return <Card><h3>Exceptions</h3><Button onClick={async () => { setDetecting(true); await detectExceptions(); setLoading(true); await load(); setDetecting(false); }}>{detecting ? 'Detecting…' : 'Detect exceptions'}</Button>
    {loading ? <p>Loading…</p> : null}
    {error ? (error === 'Not logged in' ? <LoginPrompt /> : <p>Error: {error}</p>) : null}
