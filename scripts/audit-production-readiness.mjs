...
const topFixes = [
  ["1", "Attach disposable-database and browser proof for explicit Control Tower degraded states", "Operations/Control Tower", "Diagnostic-backed failure handling is implemented but lacks current-SHA runtime and browser evidence", "Dashboards could regress to masking unavailable data", "M"],
  ["2", "Prove the requisition line-count contract on a disposable database at the current SHA", "Requisitions", "The database and in-memory stores return lineCount and the UI consumes it, but current mutation-safe runtime evidence is unavailable", "Approvers and buyers could act on incomplete demand data if the contract regresses", "S"],
  ["3", "Attach current-SHA tenant, permission, and browser evidence for RFQ and quote workflows", "Procurement", "First-class sourcing routes and services exist; their historical evidence cannot approve the current expanded source state", "Quote and compliance controls could regress without release-bound evidence", "M"],
  ...
  ["5", "Attach current-SHA runtime proof for Master Data where-used and deactivation blockers", "Master Data", "Where-used APIs and server-side deactivate/delete blockers exist, but the disposable-database suite cannot run in the current environment", "Dependency enforcement could regress without release-bound evidence", "M"],
  ["6", "Attach current-SHA service evidence for supplier, item, tax, GL, FX, and UOM validation", "Procurement", "Authoritative validation paths exist, but their mutation-safe runtime matrix has not run against this source state", "Invalid POs could enter the procurement flow if validation regresses", "M"],
  ["7", "Complete current-SHA payment approval and supplier-banking role evidence", "Finance/AP", "Sensitive actions have permission controls and expanded role tests, but disposable-database execution is unavailable", "Unauthorized users may view or trigger sensitive finance actions", "M"],
