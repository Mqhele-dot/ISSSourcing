 ...
 * Resets FQA POs via `seed:functional-qa`, then mutates PO-FQA-001 (open→approved→sent).
 *
...
async function main() {
  ...
  const get2 = await apiJsonRequest("/purchase/orders/PO-FQA-002", { cookie: sessionCookie });
  if (!get2.ok) {
  ...
  const get3 = await apiJsonRequest("/purchase/orders/PO-FQA-003", { cookie: sessionCookie });
  if (!get3.ok) {
  ...
  const actHuge = await apiJsonRequest("/activity?limit=999", { cookie: sessionCookie });
  ...
  const actDefault = await apiJsonRequest("/activity", { cookie: sessionCookie });
