...
export function registerProcurementCompletionRoutes(app: Express, auth: AuthBundle): void {
      ...
      const result = await pool.query(
        `WITH
         ...
         kpis AS (SELECT
           ...
           (SELECT count(*) FROM po WHERE upper(status) NOT IN ('RECEIVED','COMPLETED','CLOSED','CANCELLED'))::int AS "openPurchaseOrders",
           (SELECT count(*) FROM po LEFT JOIN latest_confirmation c ON c.purchase_order_id=po.id WHERE upper(po.status) IN ('SENT','ACKNOWLEDGED') AND COALESCE(c.status,'AWAITING')='AWAITING')::int AS "awaitingConfirmation",
           (SELECT count(*) FROM po WHERE upper(status)='PARTIALLY_RECEIVED')::int AS "partiallyReceived",
           (SELECT count(*) FROM po WHERE expected_delivery_date < now() AND upper(status) NOT IN ('RECEIVED','COMPLETED','CLOSED','CANCELLED'))::int AS "lateDeliveries",
           ...
           (SELECT count(*) FROM invoices i WHERE i.organization_id=$1 AND i.purchase_order_id IS NOT NULL AND upper(i.status) IN ('DRAFT','DISPUTED','PENDING_APPROVAL'))::int AS "unmatchedInvoices")
      ...
      const result = await withTransaction(async (client) => {
        ...
        const missing = await client.query(
          `SELECT po.id,po.order_number,po.supplier_id FROM purchase_orders po
           ...
           WHERE po.organization_id=$1 AND upper(po.status) IN ('SENT','ACKNOWLEDGED')
        ...
        const late = await client.query(`SELECT id,order_number,supplier_id FROM purchase_orders WHERE organization_id=$1 AND expected_delivery_date<now() AND upper(status) NOT IN ('RECEIVED','COMPLETED','CLOSED','CANCELLED')`, [org]);
        ...
        const resolved = await client.query(
          `UPDATE operational_exceptions e SET status='resolved',resolved_at=now(),updated_at=now()
           WHERE e.organization_id=$1 AND e.status IN ('open','in_progress') AND e.related_refs->>'area'='procurement'
             AND ((e.type='supplier_confirmation_overdue' AND NOT EXISTS (SELECT 1 FROM purchase_orders po LEFT JOIN LATERAL (SELECT status FROM po_supplier_confirmations c WHERE c.organization_id=$1 AND c.purchase_order_id=po.id ORDER BY c.created_at DESC,c.id DESC LIMIT 1) c ON true WHERE po.id=(e.related_refs->>'purchaseOrderId')::int AND po.organization_id=$1 AND upper(po.status) IN ('SENT','ACKNOWLEDGED') AND COALESCE(c.status,'AWAITING')='AWAITING'))
               OR (e.type='purchase_order_late' AND NOT EXISTS (SELECT 1 FROM purchase_orders po WHERE po.id=(e.related_refs->>'purchaseOrderId')::int AND po.organization_id=$1 AND po.expected_delivery_date<now() AND upper(po.status) NOT IN ('RECEIVED','COMPLETED','CLOSED','CANCELLED'))
        ...
        return { createdOrRetained: missing.rowCount + late.rowCount + noQuotes.rowCount, resolved: resolved.rowCount, rules: { overdueConfirmation: missing.rowCount, latePurchaseOrder: late.rowCount, rfqNoQuotes: noQuotes.rowCount } };
      });
      ...
      const result = await pool.query(
        `SELECT po.id,po.order_number AS "number",po.status,po.approval_status AS "approvalStatus",
          CASE WHEN upper(po.approval_status) NOT IN ('APPROVED') THEN 'Complete approval'
               WHEN upper(po.status) IN ('DRAFT','OPEN') THEN 'Send to supplier'
               WHEN upper(po.status) IN ('SENT','ACKNOWLEDGED') AND COALESCE(c.status,'AWAITING')='AWAITING' THEN 'Await supplier confirmation'
               WHEN upper(po.status) IN ('SENT','ACKNOWLEDGED','PARTIALLY_RECEIVED') THEN 'Receive goods or services'
               WHEN upper(po.status)='RECEIVED' THEN 'Close purchase order' ELSE 'No action' END AS "nextAction",
  ...
  app.get("/api/v2/procurement/receipts/:id", ...read, async (req, res) => {
    try {
      ...
      const [receipt, lines, reversals] = await Promise.all([
        pool.query(`SELECT r.*,po.order_number,s.name AS supplier_name FROM ap_receipts r JOIN purchase_orders po ON po.id=r.purchase_order_id AND po.organization_id=$1 LEFT JOIN suppliers s ON s.id=r.supplier_id AND s.organization_id=$1 WHERE r.organization_id=$1 AND r.id=$2`, [org, id]),
        pool.query(`SELECT ri.*,ii.sku,ii.name AS item_name,poi.quantity AS ordered_quantity,poi.received_quantity FROM ap_receipt_items ri JOIN ap_receipts r ON r.id=ri.receipt_id AND r.organization_id=$1 JOIN inventory_items ii ON ii.id=ri.item_id AND ii.organization_id=$1 LEFT JOIN purchase_order_items poi ON poi.id=ri.purchase_order_item_id WHERE ri.receipt_id=$2 ORDER BY ri.id`, [org, id]),
  ...
  app.post("/api/v2/procurement/contracts/:id/approve", ...approve, async (req, res) => {
    try {
      ...
      const result=await pool.query(`UPDATE supplier_contracts SET approval_status='APPROVED',status='active',owner_user_id=COALESCE(owner_user_id,$1),version=version+1,updated_at=now() WHERE id=$2 AND organization_id=$3 AND approval_status IN ('DRAFT','PENDING_APPROVAL') AND start_date<=now() AND (end_date IS NULL OR end_date>=now()) RETURNING *`,[userId||null,id,org]);
      if(!result.rows[0]) return sendError(res,409,"CONTRACT_APPROVAL_BLOCKED","Contract must be current and pending approval."); return sendOk(res,result.rows[0]);
    } catch(error){return handleError(res,error);}
  });
  ...
  app.post("/api/v2/finance/budgets/:id/activate",...admin,async(req,res)=>{try{const result=await pool.query(`UPDATE finance_budgets SET status='ACTIVE',updated_at=now() WHERE id=$1 AND organization_id=$2 AND status='DRAFT' RETURNING *`,[positiveInt(req.params.id,0),getActiveOrganizationId()]);if(!result.rows[0])return sendError(res,409,"BUDGET_ACTIVATION_BLOCKED","Only a tenant-owned draft budget can be activated.");return sendOk(res,result.rows[0]);}catch(error){return handleError(res,error);}});
