...
import { loadSecurityReleaseStatus } from "./lib/security-release-status";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/diagnostics/snapshot", async (_req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        securityReleaseStatus: loadSecurityReleaseStatus(),
  ...
  app.get("/api/diagnostics/security-release-status", async (_req: Request, res: Response) => {
    try {
      return res.json(loadSecurityReleaseStatus());
    } catch (error) {
      recordServerDiagnosticEvent({
        severity: "error",
        source: "security",
        title: "Security release status load failed",
        message: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined,
        details: error,
      });
      return res.status(500).json({ message: "Security release status failed" });
    }
  });
