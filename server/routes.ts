...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/invoices", async (req, res) => {
    try {
      ...
      const now = new Date();
      if (typeof invoiceData.issueDate === "string" && invoiceData.issueDate.trim()) {
        const parsedIssueDate = new Date(invoiceData.issueDate);
        if (!Number.isNaN(parsedIssueDate.getTime())) {
          invoiceData.issueDate = parsedIssueDate;
        }
      }
      if (typeof invoiceData.dueDate === "string" && invoiceData.dueDate.trim()) {
        const parsedDueDate = new Date(invoiceData.dueDate);
