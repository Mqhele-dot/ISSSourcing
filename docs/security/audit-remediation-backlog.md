...
Run from repo root (CI uses **Node 24**; local dev should use Node 22.12+ per `engines`):
...
**Direct** npm dependencies implicated at **high+** included: `cloudinary`, `drizzle-orm`, `electron`, `electron-builder`, `express`, **`multer-storage-cloudinary`** (dependency removed in favor of inline Multer to Cloudinary stream upload), `nodemailer`, `sqlite3`.
...
Measurable drop in high/critical count with **documented before/above table** and **after** rows. Preserve **`npm run verify:release`** / **`npm run release:gate:delta`** semantics unless deliberate policy change.
