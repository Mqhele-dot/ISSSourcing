...
export function withDeveloperToolBoundary<P extends object>(Component: ComponentType<P>) {
  return function DeveloperToolBoundary(props: P) {
    ...
    if (capabilities.isError) return <Card className="mx-auto mt-8 max-w-xl"><CardHeader><CardTitle>Developer tools unavailable</CardTitle><CardDescription>The deployment capability could not be verified.</CardDescription></CardHeader><CardContent><Button variant="outline" onClick={() => void capabilities.refetch()}>Retry</Button></CardContent></Card>;
    if (capabilities.data?.developerToolsEnabled !== true) return <Card className="mx-auto mt-8 max-w-xl"><CardHeader><CardTitle>Developer tools disabled</CardTitle><CardDescription>These diagnostics are available only to administrators in explicitly enabled non-production deployments.</CardDescription></CardHeader></Card>;
