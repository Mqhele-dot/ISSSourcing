...
export function normalizePurchaseOrderDetail(raw: unknown): PurchaseOrderDetail {
  ...
  return {
    ...
    currencyCode: pickStr(r.currencyCode ?? r.currency_code, "ZAR").toUpperCase(),
    reportingCurrencyCode: pickStr(r.reportingCurrencyCode ?? r.reporting_currency_code, "ZAR").toUpperCase(),
    reportingExchangeRate: pickNumOrNull(r.reportingExchangeRate ?? r.reporting_exchange_rate),
    reportingTotal: pickNumOrNull(r.reportingTotal ?? r.reporting_total),
    lines,
    shipments,
