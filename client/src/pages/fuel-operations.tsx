...
type FuelTank = { id: number; stationId: number; code: string; productId?: number | null; productType: string; storageType: string; capacityLitres: number; currentLevelLitres: number; reorderLevelLitres: number; status: string };
type FuelProduct = { id: number; code: string; name: string; productClass: string; unit: string; applicableStorageTypes: string[]; active: boolean };
...
function FuelActionDialog({ action, setAction, workspace }: { action: FuelAction | null; setAction: (action: FuelAction | null) => void; workspace: FuelWorkspace }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const requests: Record<FuelAction, { method: "POST"; url: string; body: Record<string, unknown> }> = {
        ...
        tank: { method: "POST", url: "/api/fuel/tanks", body: { ...commonStation, code: form.code, productId: n("productId"), storageType: form.storageType, capacityLitres: n("capacityLitres"), currentLevelLitres: n("currentLevelLitres"), reorderLevelLitres: n("reorderLevelLitres") } },
        ...
        price: { method: "POST", url: "/api/fuel/prices", body: { ...commonStation, productId: n("productId"), pricePerLitre: n("pricePerLitre"), effectiveFrom: form.effectiveFrom } },
  ...
  return <Dialog open={action != null} onOpenChange={(open) => !open && setAction(null)}><DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>{action ? ACTION_LABELS[action] : "Fuel operation"}</DialogTitle><DialogDescription>Values are validated against the active organization, selected station, and registered storage capacity.</DialogDescription></DialogHeader><form className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}>
    ...
    {action === "tank" ? <>{stationSelect}<Field label="Tank code" name="code" form={form} setForm={setForm} /><NativeSelect label="Product" name="productId" value={form.productId} onChange={set("productId")} disabled={!workspace.products.some((product) => product.active)}><option value="">Select product</option>{workspace.products.filter((product) => product.active).map((product) => <option key={product.id} value={product.id}>{product.name} ({product.unit})</option>)}</NativeSelect><NativeSelect label="Storage type" name="storageType" value={form.storageType} onChange={set("storageType")}><option value="underground_tank">Underground tank</option><option value="above_ground_tank">Above-ground tank</option><option value="lpg_bulk_tank">LPG bulk tank</option></NativeSelect><Field label="Capacity (L)" name="capacityLitres" type="number" min="0.01" step="0.01" form={form} setForm={setForm} /><Field label="Current level (L)" name="currentLevelLitres" type="number" min="0" step="0.01" form={form} setForm={setForm} /><Field label="Reorder level (L)" name="reorderLevelLitres" type="number" min="0" step="0.01" form={form} setForm={setForm} /></> : null}
    ...
    {action === "price" ? <>{stationSelect}<NativeSelect label="Product" name="productId" value={form.productId} onChange={set("productId")} disabled={!workspace.products.some((product) => product.active)}><option value="">Select product</option>{workspace.products.filter((product) => product.active).map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</NativeSelect><Field label="Selling price per litre" name="pricePerLitre" type="number" min="0.01" step="0.01" form={form} setForm={setForm} /><Field label="Effective from" name="effectiveFrom" type="datetime-local" form={form} setForm={setForm} /></> : null}
