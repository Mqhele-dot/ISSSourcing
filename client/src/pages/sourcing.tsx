...
function EventDetail({ eventId }: { eventId: number }) {
  ...
  return <PageDataState isLoading={detailsQuery.isLoading} error={detailsQuery.error} isEmpty={!detailsQuery.data} onRetry={() => void detailsQuery.refetch()} emptyView={<div>RFQ not found.</div>}>
    {detailsQuery.data ? <div className="space-y-5" data-testid="sourcing-event-detail">
      ...
      {detailsQuery.data.event.status === "EVALUATING" && comparisonQuery.data?.length ? <EvaluationAndAwardPanel eventId={eventId} details={detailsQuery.data} comparison={comparisonQuery.data} /> : null}
    </div> : null}
  </PageDataState>;
}
...
export default function SourcingPage() {
  ...
  const eventsQuery = useQuery<SourcingEvent[]>({ queryKey: qk.sourcing, queryFn: () => requestJson("GET", "/api/sourcing/events"), enabled: !match });
  const events = useMemo(() => (eventsQuery.data ?? []).filter((event) => `${event.eventNumber} ${event.title} ${event.status}`.toLowerCase().includes(search.toLowerCase())), [eventsQuery.data, search]);
  useEffect(() => {
