...
export default function SystemDiagnosticsPage() {
  ...
  const securityReleaseStatus = snapshot?.securityReleaseStatus;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6" data-testid="system-diagnostics-page">
      ...
      <Card data-testid="diagnostics-security-status">
        <CardHeader>
          <CardTitle className="text-base">Security and release status</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-sm">
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            <SummaryTile label="Overall">{securityReleaseStatus?.summary?.overall ?? "unknown"}</SummaryTile>
            <SummaryTile label="Status source">{securityReleaseStatus?.source ?? "none"}</SummaryTile>
            <SummaryTile label="Audit high/critical">
              {securityReleaseStatus?.audit
                ? `${securityReleaseStatus.audit.high} high / ${securityReleaseStatus.audit.critical} critical`
                : "not captured"}
            </SummaryTile>
            <SummaryTile label="Blocking gates">
              {securityReleaseStatus?.summary?.blockingGates?.length
                ? securityReleaseStatus.summary.blockingGates.join(", ")
                : "none recorded"}
            </SummaryTile>
          </div>
          {securityReleaseStatus?.nextAction ? (
            <p className="rounded-md border border-border bg-muted/30 p-2 text-xs">
              <span className="font-medium text-foreground">Next action: </span>
              {securityReleaseStatus.nextAction}
            </p>
          ) : null}
          {securityReleaseStatus?.summary?.blockingReasons?.length ? (
            <div className="rounded-md border border-amber-500/40 bg-amber-500/5 p-3 text-xs">
              <p className="font-medium text-foreground">Blocking reasons</p>
              <ul className="mt-2 list-disc space-y-1 pl-4 text-muted-foreground">
                {securityReleaseStatus.summary.blockingReasons.map((reason) => (
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {Object.entries(securityReleaseStatus?.gates ?? {}).map(([key, gate]) => (
              <div key={key} className="rounded-md border p-2">
                ...
                {gate.details ? <p className="mt-1 text-xs text-muted-foreground">{gate.details}</p> : null}
              </div>
            ))}
          </div>
          {securityReleaseStatus?.audit?.topFindings?.length ? (
            <div className="rounded-md border p-3">
              <p className="font-medium">Top audit findings</p>
              <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-muted-foreground">
                {securityReleaseStatus.audit.topFindings.map((finding) => (
                  <li key={`${finding.package}-${finding.severity}`}>
                    {finding.package} ({finding.severity}){finding.via.length ? `: ${finding.via.join(", ")}` : ""}
                  </li>
                ))}
              </ul>
            </div>
